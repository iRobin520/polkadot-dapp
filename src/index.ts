...
export class PolkadotDappHelper {
    coinType: string;
    address: string;
     ...
     * @param config
     */
    setConfig = (config?: {coinType: string, address: string}) => {
     ...
     */
    callNativeMethod = function (method, id, params, callback) {
        const _this = this;
        const methodName = 'dot.' + method;
        const parameters = {'name':method, 'id':id, 'object':params};
            ...
            try {
                if (response) {
                    ...
                    if (!error && result) {
                        ...
                        callbackBody = _this.generateCallbackBody(id,result);
                    } else {
     ...
     */
    generateCallbackBody = function(id,response) {
        return {
            jsonrpc: "2.0",
            id: id,
            result: response
     ...
     * @param {function} callback
     */
    requestAccounts = function(coinType, callback) {
     ...
     * @param {function} callback
     */
    requestAllAccounts = function(coinType, callback) {
     ...
     * @param {function} callback
     */
    requestKeyringPair = function(callback) {
...
// }
//
// PolkadotDappHelper.prototype.generateMnemonic = function () {
